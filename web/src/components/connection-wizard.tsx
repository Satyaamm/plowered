          ...
          <div className={styles.section}>
            ...
            {type === "postgres" && (
              <>
                <div className={styles.rowGrid}>
                  <Field
                    label={
                      <InfoLabel info="Hostname or IP of the Postgres server reachable from Plowered workers. For managed databases, the endpoint shown in your cloud console.">
                        Host
                      </InfoLabel>
                    }
                    required
                ...
                <Field
                  label={
                    <InfoLabel info="Role used by Plowered to read metadata and run quality checks. Read-only suffices; we recommend a dedicated 'plowered' role with SELECT + USAGE only.">
                      Username
                    </InfoLabel>
                  }
                  required
                ...
                <Field
                  label={
                    <InfoLabel info="TLS strictness for the Postgres connection. 'require' enforces TLS without cert verification; 'verify-full' is the strongest. Use 'disable' only for localhost dev.">
                      SSL mode
                    </InfoLabel>
                  }
                >
            ...
            {type === "snowflake" && (
              <>
                <Field
                  label={
                    <InfoLabel info="Snowflake account locator: <orgname>-<account> or <account>.<region>.<cloud>. Example: xy12345.us-east-1. Find it in the URL of your Snowflake console.">
                      Account
                    </InfoLabel>
                  }
                  required
                ...
                <Field
                  label={
                    <InfoLabel info="Snowflake user that Plowered authenticates as. Recommend a dedicated PLOWERED user with USAGE on warehouse + read on target schemas.">
                      Username
                    </InfoLabel>
                  }
                  required
                ...
                <div className={styles.rowGrid}>
                  <Field
                    label={
                      <InfoLabel info="Compute warehouse used for crawls, samples, and quality checks. Pick a small or X-small warehouse — Plowered's workload is metadata-heavy, not compute-heavy.">
                        Warehouse
                      </InfoLabel>
                    }
                  >
            ...
            {type === "bigquery" && (
              <>
                <Field
                  label={
                    <InfoLabel info="The GCP project that owns the datasets you want catalogued. Plowered's service account needs roles/bigquery.metadataViewer + dataViewer on this project.">
                      Project ID
                    </InfoLabel>
                  }
                  required
                ...
                <Field
                  label={
                    <InfoLabel info="Service account JSON: paste a key with bigquery.metadataViewer + dataViewer. Workload identity: bind Plowered's GKE/Cloud Run SA to a GCP service account (no key material stored).">
                      Auth method
                    </InfoLabel>
                  }
                >
