...
| **Auth** | Email + password signup, email verification, sessions (HttpOnly cookies), invite-based teammate onboarding, 5-role RBAC |
| **Orchestration** | Pipelines, scheduled runs, run logs, quality checks, audit log |
| **Async jobs** | Asynq (Redis Streams) or in-process sync mode. Visible per-tenant via `GET /v1/jobs/{id}` |
| **Connectors** | Postgres (live), Snowflake (live), BigQuery (scaffold; activate by registering the client driver) |
...
| **Compliance** | Legal holds, DSR (data-subject requests), audit trail, recycle bin |
...
| AuthN | Sessions (HttpOnly cookie). OIDC adapter slot reserved. |
...
- **DSR**: per-subject delete/export flows tracked with statutory clocks.
- **Audit**: every write is recorded in `audit_log`; retention is per-tenant.
...
The seven feature tracks shipped most recently:
